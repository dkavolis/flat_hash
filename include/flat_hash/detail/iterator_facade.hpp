...
[[nodiscard]] ITERF_ALWAYS_INLINE
    constexpr auto arrow_helper(T&& t) noexcept(std::is_nothrow_move_constructible_v<std::remove_cvref_t<T>>)
        -> arrow_proxy<std::remove_cvref_t<T>> {
  return {std::move(t)};
}
